 ...
 */
package io.reactivesocket.netty.udt.server;
...
public class ReactiveSocketServerHandler extends SimpleChannelInboundHandler<UdtMessage> {
    private Logger logger = LoggerFactory.getLogger(ReactiveSocketServerHandler.class);

    private ConcurrentHashMap<ChannelId, ServerUdtDuplexConnection> duplexConnections = new ConcurrentHashMap<>();
    ...
    protected void channelRead0(ChannelHandlerContext ctx, UdtMessage msg) throws Exception {
        ...
        ServerUdtDuplexConnection connection = duplexConnections.computeIfAbsent(ctx.channel().id(), i -> {
            System.out.println("No connection found for channel id: " + i);
            ServerUdtDuplexConnection c = new ServerUdtDuplexConnection(ctx);
